...
/**
 * @extends Factory<RentalSaleOrder>
 */
class RentalSaleOrderFactory extends Factory
{
    /**
    ...
    public function definition(): array
    {
        ...
        if (SoRentalType::LONG_TERM === $rental_type) {
            $rental_payment_type      = SoRentalPaymentType::label_key_random();
            $rental_payment_type_rule = SoRentalPaymentType::interval[$rental_payment_type]; // ['interval_unit']

            /** @var SoPaymentDay_Month $payment_day_class */
            $payment_day_class = SoRentalPaymentType::payment_day_classes[$rental_payment_type];
            $payment_day       = $payment_day_class::label_key_random();

            $installments = $this->faker->numberBetween(10, 60);

            $rental_end = Carbon::create($rental_start)->modify(sprintf('+ %d %s', $installments * $rental_payment_type_rule['interval'], $rental_payment_type_rule['interval_unit']));
        ...
        return [
            ...
            'rental_payment_type'             => $rental_payment_type ?? null,
