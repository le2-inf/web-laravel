...
#[ColumnDesc('rental_payment_type', required: true, enum_class: SoRentalPaymentType::class)]
#[ColumnDesc('cu_id')]
 ...
 * @property null|SoRentalPaymentType|string        $rental_payment_type                 付款方式；例如月付预付、月付后付等
 * @property null|string                            $rental_payment_type_label           付款类型-中文
...
class RentalSaleOrder extends Model
{
    ...
    protected $casts = [
        ...
        'rental_payment_type'  => SoRentalPaymentType::class,
     ...
     * 签约时需要支付的.
     */
    public function SignRentalPayments(): HasMany
    {
        return $this->hasMany(RentalPayment::class, 'so_id', 'so_id')
     ...
     * 未支付的租金列表.
     */
    public function UnpaidRentRentalPayments(): HasMany
    {
        return $this->hasMany(RentalPayment::class, 'so_id', 'so_id')
    ...
    public function RentalSaleSettlement(): HasOne
    {
        return $this->hasOne(RentalSaleSettlement::class, 'so_id', 'so_id');
    }
    ...
    public function payments_phpword_func($tp, $rule_label)
    {
        ...
        foreach ($this->RentalPayments as $payment) {
            $table->addRow();
            $table->addCell()->addText($payment->RentalPaymentType->pt_name, [], ['alignment' => Jc::CENTER]);
    ...
    {
        return $this->belongsTo(RentalCompany::class)->withDefault(
            RentalCompany::query()->firstOrNew()->toArray()
        );
    }
    ...
    {
        return [
            'rental_type'                     => [RentalSaleOrder::class, 'rental_type'],
            'rental_payment_type'             => [RentalSaleOrder::class, 'rental_payment_type'],
            ...
            'rental_start'                    => [RentalSaleOrder::class, 'rental_start'],
            'installments'                    => [RentalSaleOrder::class, 'installments'],
            'rental_end'                      => [RentalSaleOrder::class, 'rental_end'],
            'deposit_amount'                  => [RentalSaleOrder::class, 'deposit_amount'],
            'management_fee_amount'           => [RentalSaleOrder::class, 'management_fee_amount'],
            'rent_amount'                     => [RentalSaleOrder::class, 'rent_amount'],
            'payment_day'                     => [RentalSaleOrder::class, 'payment_day'],
            'total_rent_amount'               => [RentalSaleOrder::class, 'total_rent_amount'],
            'insurance_base_fee_amount'       => [RentalSaleOrder::class, 'insurance_base_fee_amount'],
            'insurance_additional_fee_amount' => [RentalSaleOrder::class, 'insurance_additional_fee_amount'],
            'other_fee_amount'                => [RentalSaleOrder::class, 'other_fee_amount'],
            'total_amount'                    => [RentalSaleOrder::class, 'total_amount'],
            'order_status'                    => [RentalSaleOrder::class, 'order_status'],
            'order_at'                        => [RentalSaleOrder::class, 'order_at'],
            'signed_at'                       => [RentalSaleOrder::class, 'signed_at'],
            'canceled_at'                     => [RentalSaleOrder::class, 'canceled_at'],
            'completed_at'                    => [RentalSaleOrder::class, 'completed_at'],
            'early_termination_at'            => [RentalSaleOrder::class, 'early_termination_at'],
        ];
    }
    ...
    public static function importBeforeValidateDo(): \Closure
    {
        return function (&$item) {
            ...
            $item['cu_id']               = RentalCustomer::plateNoKv($item['contact_phone'] ?? null);
            $item['rental_type']         = SoRentalType_Short::searchValue($item['rental_type'] ?? null);
            $item['rental_payment_type'] = SoRentalPaymentType::searchValue($item['rental_payment_type'] ?? null);
    ...
    public static function importValidatorRule(array $item, array $fieldAttributes): void
    {
        ...
        $rules = [
            ...
            'contract_number' => ['bail', 'required', 'string', 'max:50', Rule::unique(RentalSaleOrder::class, 'contract_number')],
    ...
    public static function importAfterValidatorDo(): \Closure
    {
        return function () {
            // contract_number
            $contract_number = RentalSaleOrder::query()->whereIn('contract_number', static::$fields['contract_number'])->pluck('contract_number')->toArray();
    ...
    public static function importCreateDo(): \Closure
    {
        return function ($input) {
            $rentalSaleOrder = RentalSaleOrder::query()->create($input);
    ...
    protected function rentalPaymentTypeLabel(): Attribute
    {
        return Attribute::make(
            get : fn () => $this->getAttribute('rental_payment_type')?->label
        );
    }
    ...
    protected function paymentDayLabel(): Attribute
    {
        return Attribute::make(
            get: function () {
                $rental_payment_type = $this->getAttribute('rental_payment_type');
                if (null === $rental_payment_type) {
                    return null;
                }
                $map = str_starts_with('week', $rental_payment_type) ? SoPaymentDay_Week::LABELS : SoPaymentDay_Month::LABELS;
