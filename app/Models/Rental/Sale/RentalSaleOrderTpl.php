...
use App\Enum\Rental\SotRentalPaymentType;
 ...
 * @property mixed       $rental_payment_type             付款类型；例如月付预付、月付后付等
 ...
 */
class RentalSaleOrderTpl extends Model
{
    use ModelTrait;
    ...
    {
        return DB::query()
            ->from('rental_sale_order_tpls', 'sot')
            ->orderByDesc('sot.sot_id')
            ->select('sot.*')
            ->addSelect(
                DB::raw(SotRentalType::toCaseSQL()),
                DB::raw(SotRentalPaymentType::toCaseSQL()),
    ...
    protected function paymentDayLabel(): Attribute
    {
        return Attribute::make(
            get: function () {
                ...
                $class = SoRentalPaymentType::payment_day_classes[$rental_payment_type->value];
